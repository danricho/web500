...
#
# 1. dev_random_* - THE ADMIN TEST-MODE BOT ONLY: DELIBERATELY DUMB AND PURELY RANDOM,
...
#   ADMIN TEST-MODE BOT: "D|CLUNKER" (NAME FORCED UPPERCASE - VISUALLY LOUD IN UI/LOGS)
...
# ADMIN: SEATS RANDOMLY-CHOSEN DEV BOTS IN EMPTY SEATS - QUEUED WHEN TEST MODE IS ENABLED
...
# ADMIN: PERFORMS ONE PURELY RANDOM ACTION FOR THE FOCUSED DEV BOT - QUEUED BY sio_push IN TEST MODE
...
def player_bot_check(game):
  ...
  # DECISION USES CURRENT STATE. skip_delays (ADMIN) BYPASSES AS ALWAYS
...
# ADMIN TEST MODE AND PLAYER BOTS CAN EVEN COEXIST (D|/B| PREFIXES KEEP NAMES DISTINCT).
def bot_check(game):
  if game.test_mode:
